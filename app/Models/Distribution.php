 ...
 *        
 */
class Distribution extends PpciModel
{
	...
	function write($data): int
	{
		...
		if ($id > 0) {
			...
			if (!isset($this->repartAliment)) {
				$this->repartAliment = new RepartAliment;
			}
			...
			if (!isset($this->distribQuotidien)) {
				$this->distribQuotidien = new DistribQuotidien;
			}
			if (!isset($this->alimentQuotidien)) {
				$this->alimentQuotidien = new AlimentQuotidien;
			}
			/*
	 ...
	 * @return integer
	 */
	function writeReste(array $data)
	{
		$erreur = 0;
		/*
