 ...
 *        
 */
class Bassin extends PpciModel
{
	/**
	...
	function generateWhere(array $dataSearch): void
	{
		...
		if ($dataSearch["bassin_type"] > 0) {
			$this->where .= $and . " bassin_type_id = :bassin_type:";
			$this->dataRequest["bassin_type"] = $dataSearch["bassin_type"];
			$and = " and ";
		}
		...
		if ($dbassin["mode_calcul_masse"] == 0) {
			...
			if ($nbPoissonTotal > 0) {
				...
				foreach ($listePoisson as $poisson) {
					$ids .= $comma . ":pi_$i:";
					$a_ids["pi_" . $i] = $poisson["poisson_id"];
					$comma = ",";
					$i++;
				}
				/**
				...
				$dataMasse = $this->lireParam($sql, $a_ids);
				if ($dataMasse["nb"] > 0) {
