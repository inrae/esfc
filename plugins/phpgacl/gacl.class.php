...
/*
	function gacl($db,$prefixe) {
		$this->_db = $db;
		$this->_db_table_prefix = $prefixe;
...
*/	
	...
	$this->config_file = dirname(__FILE__)."/../../param/gacl.ini.php";
//	echo $this->config_file;
		$available_options = array('db','debug','items_per_page','max_select_box_items','max_search_return_items','db_table_prefix','db_type','db_host','db_user','db_password','db_name','caching','force_cache_expire','cache_dir','cache_expire_time','smarty_dir','smarty_template_dir','smarty_compile_dir');
		...
		if (is_object($this->_db)) {
			...
			$etatconn=$this->db->PConnect($this->_db_host, $this->_db_user, $this->_db_password, $this->_db_name);
			if ($etatconn == FALSE) {
		echo( "Echec de la connexion à la base de données GACL" );
		die ();
	}
