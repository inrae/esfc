...
class ObjetBDD {
	 ...
	 * non numerique, 1 pour numerique, 2 pour date, 3 pour un datetime) vous pouvez ne decrire
	 * que les champs numeriques et dates de la table structure : "champ"
	 * =>type"
	 */
	public $types;
	/**
	 ...
	 *
	 * @var array
	 * Tableau contenant les parametres passes lors de la construction de la classe
	...
	function __construct(&$p_connection, $param=NULL)
	{
				...
				foreach($value as $key1 => $value1) {
					switch($key1) {
						...
						case "pattern":
							...
							break;
						case "requis":
							if ($value1==1) $this->champs_nonvides[]=$key;
							break;
						case "defaultValue":
		...
		*/
		if ($this->formatDate == "fr") $this->formatDate = 1;
		if ($this->formatDate == "en") $this->formatDate = 2;
	}
	 ...
	 * @param array $param
	 * @return void
	 * Fonction permettant de forcer les parametres a utiliser
	 */
	...
	private function execute($sql){
		...
		if ((!$rs && $this->debug_mode==1)||$this->debug_mode==2) print($this->connection->ErrorMsg()."<br>".$sql);
	 ...
	 * @return boolean|Ambigous <boolean, $data, string>
	 */
	function lire($id, $getDefault = false, $parentValue = 0)
	{
		...
		if ($this->cleMultiple==1) {
			...
			if ($this->verifData){
				if ($this->verifDonnees($id) == false) return false;
			}
			$where = "";
			...
			*/
			if ($this->verifData) {
				if ($this->verifDonnees($this->cle) == false )return false;
			}
			$where = $this->cle. ' = '.$id;
		}
	 ...
	 * @param   string - commande sql a executer
	 ...
	 * @param :integer - cle de l'enregistrement a supprimer
	 ...
	 * @param  integer - identifiant concerne
	 * @param  string - nom du champ sur lequel porte la requete
	 *
	 */
	function supprimerChamp($id,$champ/*:int*/)/*:int*/
	{
		...
		if ($id > 0) {
			return $this->execute("delete from ".$this->table." where ".$champ."=".$id);
		}
	}
	/**
	...
	/**
	 * Function ecrire
	 * @param  array with the name of the columns as identifiers of items
	...
	function ecrire($dataBrute) {
		...
		*/
		if ($this->cleMultiple==0 && $data[$this->cle]=="" &&$this->id_auto>0) $data[$this->cle]=0;
		/*
		 * Rajout des slashes devant les quotes et autres caracteres concernes pour eviter
		* les attaques par injection de code, et accessoirement autoriser la saisie de
		* guillemets doubles
		*/
		if (get_magic_quotes_gpc()==0) {
			foreach($data as $key=>$value) {
				if ($this->typeDatabase =='postgre') {
					if ($this->UTF8 == true) {
						$data[$key] = pg_escape_string(mb_convert_encoding($value, 'UTF-8'));
					} else {
		...
		*/
		...
		if ($this->verifData){
			if ($this->verifDonnees($data,$mode) == false) return false;
		}
		/*
		...
		if($mode == "ajout" )
		{
			...
			foreach ($data as $key => $value)
			{
				...
				if ($this->id_auto==1&&$key==$this->cle) {
					...
					$sqlmax = 'select max('.$this->cle.') from '.$this->table;
	 ...
	 * @param string - code de la requete SQL
	 ...
	 * @param  collection equivalente a $this->types
	 * @param  array contenant les valeurs a traiter (l'enregistrement en cours)
	 ...
	 *
	 * @param string $date
	 * @param int $type ; 2 : date, 3 : datetime
	 * @return string
	 */
	function formatDateLocaleVersDB ($date, $type=2) {
		...
		do
		{
			...
			if($test === false) $j++;  //!\Important le triple egal verifie que c'est bien la valeur false qui est retournee
	 ...
	 * @return boolean
	 * Fonction permettant de verifier les donnees avant ecriture en base de donnees.
	 */
	 ...
	 * @param $format : si vaut 1, le resultat est retourne sous forme de texte, avec saut de ligne
		...
		if ($format == 1) {
			...
			foreach ($this->errorData as $key => $value) {
				if ($this->errorData[$key]["code"]==0) {
					...
					$res .= "le champ ".$this->errorData[$key]["colonne"]." n'est pas numerique. Valeur saisie : ".
							$this->errorData[$key]["valeur"]."<br>";
				}elseif ($this->errorData[$key]["code"]==2) {
					$res .= "Le champ ".$this->errorData[$key]["colonne"]." est trop grand. Longueur maximale autorisée : ".
							$this->longueurs[$this->errorData[$key]["colonne"]].". Valeur saisie : ".$this->errorData[$key]["valeur"].
							" (".strlen($this->errorData[$key]["valeur"])." caracteres)<br>";
				}elseif ($this->errorData[$key]["code"]==3) {
					$res .= "Le contenu du champ ".$this->errorData[$key]["colonne"]." ne correspond pas au format attendu. Masque autorisé : ".
							$this->pattern[$this->errorData[$key]["colonne"]].". Valeur saisie : ".$this->errorData[$key]["valeur"]."<br>";
			...
			foreach ($data as $key => $value) {
				if (is_array($value)) {
					foreach ($value as $key1 => $value1) {
						...
						*/
						if ($this->param["utf8"] == true) $data[$key][$key1] = utf8_encode($data[$key][$key1]);
					}
				}else{
					$data[$key] = htmlspecialchars($value);
					if ($this->param["utf8"] == true) $data[$key] = utf8_encode($data[$key]);
				}
			}
		}else{
			$data = htmlspecialchars($data);
			if ($this->param["utf8"] == true) $data = utf8_encode($data);
		}
		return $data;
	}
			...
			foreach ($data as $key => $value) {
				if (is_array($value)) {
					foreach ($value as $key1 => $value1) {
						 ...
						 * Traitement de l'UTF8
						*/
						if ($this->param["utf8"] == true) $data[$key][$key1] = utf8_decode($data[$key][$key1]);
					}
				}else{
					$data[$key] = htmlspecialchars_decode($value);
					if ($this->param["utf8"] == true) $data[$key] = utf8_decode($data[$key]);
				}
			}
		}else{
			$data = htmlspecialchars_decode($data);
			if ($this->param["utf8"] == true) $data = utf8_decode($data);
		}
		return $data;
	}
