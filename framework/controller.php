...
while (isset($module)){
	...
	*/
	...
	$resident = 1;
	if ($t_module["loginrequis"]==1&&!isset($_SESSION["login"])) $resident = 0;
	/*
	...
	*/
	if (strlen($t_module["droits"])>1) {
		if (!isset($_SESSION["login"])) {
			...
			if ($resident == 0) $motifErreur = "droitko";
		}
	}
	...
	*/
	if (strlen($t_module["modulebefore"])>0) {
		...
		foreach ($before as $key=>$value) {
			if ($_SESSION["moduleBefore"] == $value) $beforeok = true;
		}
		if ($beforeok == false) {
			$resident = 0;
...
if ($t_module["ajax"]<>1){
	...
	*/
	if ($message == "") $message = $LANG["message"][0];
	$smarty->assign("message",$message);
	/*
	 * Gestion du menu
	* Rajout du 17/8/09 : mise en cache du menu
	*/
