...
function dataDelete($dataClass, $id, $isPartOfTransaction = false)
{
    ...
    if ($ok) {
        try {
            ...
            if ($isPartOfTransaction) {
                throw new FrameworkException($eo->getMessage());
            } else {
            ...
            if (strpos($e->getMessage(), "[23503]") !== false) {
                $mes = _("La suppression n'est pas possible : des informations sont référencées par cet enregistrement");
                if ($isPartOfTransaction) {
                    throw new FrameworkException($mes);
                } else {
                    $message->set($mes, true);
                }
            }
            if ($message->getMessageNumber() == 0 && !$isPartOfTransaction) {
